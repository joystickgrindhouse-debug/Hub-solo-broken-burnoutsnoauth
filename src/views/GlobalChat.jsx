...
// Added inline style fallback on the wrapper div so chat renders correctly even if the
// CSS file containing .hero-background is missing.
...
export default function GlobalChat({ user, userProfile, hideNavbar, roomId }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = roomId
      ? ChatService.subscribeToRoomMessages(roomId, (fetchedMessages) => setMessages(fetchedMessages), 50)
      : ChatService.subscribeToGlobalMessages((fetchedMessages) => setMessages(fetchedMessages), 50);
    return () => unsubscribe();
  }, [user, roomId]);
    ...
    <div style={containerStyle}>
      ...
      <div style={{ position: "relative", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
        ...
        {showEmojiPicker && (
          <div
            ref={emojiPickerRef}
            style={{ position: "absolute", bottom: "100%", left: "0", marginBottom: "10px", zIndex: 1000 }}
          >
            <CustomEmojiPicker
