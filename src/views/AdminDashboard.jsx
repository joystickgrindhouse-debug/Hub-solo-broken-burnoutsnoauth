...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    if (isAuthorized && user) {
      console.log("Admin Dashboard - Authorized as " + user.email + ", starting listeners...");
      ...
      <div className="min-h-screen bg-black flex items-center justify-center p-4">
        <div className="bg-zinc-900 p-8 border border-red-600 rounded-lg max-w-md w-full">
          ...
          <input 
            ...
            className="w-full bg-black border border-zinc-700 text-white p-3 rounded mb-4 focus:border-red-600 outline-none"
            value={adminKey}
            onChange={(e) => setAdminKey(e.target.value)}
          />
          <button 
            className="w-full bg-red-600 text-white font-bold py-3 rounded hover:bg-red-700 transition shadow-[0_0_15px_rgba(220,38,38,0.3)]"
            onClick={() => {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8 border-b border-zinc-800 pb-4">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
            ...
            <button type="button" onClick={() => setActiveTab("chat")} className={`px-4 py-2 rounded flex-shrink-0 transition-all font-bold text-xs tracking-widest ${activeTab === "chat" ? "bg-red-600 text-white shadow-[0_0_15px_rgba(255,48,80,0.4)]" : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700"}`}>CHAT LOGS</button>
            <button type="button" onClick={() => setActiveTab("logs")} className={`px-4 py-2 rounded flex-shrink-0 transition-all font-bold text-xs tracking-widest ${activeTab === "logs" ? "bg-red-600 text-white shadow-[0_0_15px_rgba(255,48,80,0.4)]" : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700"}`}>SYSTEM LOGS</button>
            <button type="button" onClick={() => setActiveTab("raffle")} className={`px-4 py-2 rounded flex-shrink-0 transition-all font-bold text-xs tracking-widest ${activeTab === "raffle" ? "bg-red-600 text-white shadow-[0_0_15px_rgba(255,48,80,0.4)]" : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700"}`}>RAFFLE</button>
          </div>
        </div>
        ...
        {activeTab === "users" && (
          <div className="grid gap-4">
            ...
            <div className="space-y-3">
              {users.map(u => (
                <div key={u.id} className="bg-zinc-900/50 p-4 rounded-lg border border-zinc-800 flex justify-between items-center backdrop-blur-sm transition-all hover:border-zinc-700">
                  <div className="flex items-center gap-4">
                    <div className={`w-3 h-3 rounded-full ${isUserLive(u) ? "bg-green-500 shadow-[0_0_10px_#22c55e]" : "bg-zinc-700 opacity-50"}`} />
                    <div>
              ...
              {users.length === 0 && <div className="text-center py-12 text-zinc-600 font-mono italic">Searching for users...</div>}
            </div>
          </div>
        )}
        ...
        {activeTab === "chat" && (
          <div className="bg-zinc-900/50 p-4 rounded-lg border border-zinc-800 max-h-[70vh] overflow-y-auto custom-scrollbar backdrop-blur-sm">
            <h2 className="text-zinc-500 text-[10px] font-mono tracking-widest mb-4 px-2 uppercase">Global Chat History</h2>
              ...
              {logs.map(l => (
                <div key={l.id} className={`p-3 rounded-md border ${l.type === 'error' ? 'bg-red-950/10 border-red-900/30' : 'bg-zinc-900 border-zinc-800'}`}>
                  <div className="flex justify-between items-start mb-2">
                    <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-tighter ${l.type === 'error' ? 'bg-red-600 text-white' : 'bg-blue-600 text-white'}`}>{l.type}</span>
                    <span className="text-[9px] text-zinc-500 font-mono">{l.timestamp ? new Date(l.timestamp._seconds * 1000).toLocaleString() : 'N/A'}</span>
                  </div>
        ...
        {activeTab === "raffle" && (
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-zinc-900/50 p-8 rounded-xl border border-zinc-800 text-center backdrop-blur-md">
              ...
              <button onClick={handleDrawRaffle} className="w-full bg-red-600 p-6 rounded-lg font-bold hover:bg-red-500 shadow-[0_0_25px_rgba(220,38,38,0.4)] transition-all text-lg tracking-tighter uppercase">TRIGGER SUNDAY DRAW</button>
            </div>
            <div className="bg-zinc-900/50 p-6 rounded-xl border border-zinc-800 backdrop-blur-md">
              ...
              <div className="space-y-3">
                {raffleWinners.map(w => (
                  <div key={w.id} className="bg-black/40 p-4 rounded border border-zinc-800/50 flex justify-between items-center">
                    <div>
                      <p className="text-red-500 font-bold text-sm uppercase">{w.userName}</p>
                      <p className="text-[9px] text-zinc-600 font-mono">TX_REF: {w.ticket}</p>
                    </div>
