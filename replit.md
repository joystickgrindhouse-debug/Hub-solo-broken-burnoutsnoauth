...
## Recent Changes
- **Feb 2026:** Live Mode enhancements: Added user avatars throughout (turn indicator, room browser with host crown badge + player mini-avatars, waiting-for-turn view). Discord VC link sharing in lobby (host pastes invite link, players see "Join Discord VC" button, VC shortcut in match header). `discordVcLink` stored in Firestore room data via `liveService.setDiscordLink()`. Host avatar transfer on leave. PoseVisualizer always-mounted during match (hidden via CSS, never re-mounted). Max players updated to 6.
